		...
		<tbody>
			<c:forEach items="${productDs}" var="productD">
				<tr>
					...
					<c:forEach items="${priceCategories}" var="itemP">
						<c:set var="specialPrice" value="${0}"></c:set>
						<c:forEach items="${productD.productPriceCategories}" var="itemD">
							<c:if test="${itemD.priceCategory.id==itemP.id}">
								<c:set var="specialPrice" value="${itemD.price}"></c:set>
							</c:if>
						</c:forEach>
						<td>${specialPrice}</td>
					</c:forEach>
