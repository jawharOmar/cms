...
public class ProductDAOImpl implements ProductDAOExt {
	...
	public List<ProductD> findStock() {
		...
		Query query = em.createNativeQuery("SELECT P.I_PRODUCT,P.PRODUCT_CODE,P.PRODUCT_NAME,PUT.UNIT_TYPE_NAME,\n"
				...
				+ "ROUND(SUM(PAYMENT_AMOUNT)/SUM(QUANTITY),3) as COST,P.PRICE,PC.CATEGORY_NAME AS CATEGORY,PACKET_SIZE,MINIMUM_STOCK_LEVEL\n"
		...
		for (Object row[] : resultList) {
			...
			if (row[9] != null)
				productD.setMinimumStockLevel(Integer.parseInt("" + row[9]));
	...
	public ProductD findProductByCode(String productCode) {
		...
		if (row[8] != null)
			productD.setMinimumStockLevel(Integer.parseInt("" + row[8]));
